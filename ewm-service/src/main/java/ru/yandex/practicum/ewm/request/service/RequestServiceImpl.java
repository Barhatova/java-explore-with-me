...
public class RequestServiceImpl implements RequestService {
    ...
    public ParticipationRequestDto create(Long userId, Long eventId) {
        ...
        if (event.getInitiator().equals(user)) {
            throw new RulesViolationException(String.format("User with id=%d is the owner of the event with id=%d.", userId, eventId));
        }

        if (!event.getState().equals(State.PUBLISHED)) {
            throw new RulesViolationException(String.format("Event not published. " +
        ...
        if (event.getParticipantLimit() > 0 && event.getConfirmedRequests().equals(event.getParticipantLimit())) {
            throw new RulesViolationException(String.format("The event with id=%d has reached the limit(%d) of requests for participation.", eventId, event.getParticipantLimit()));
        }
